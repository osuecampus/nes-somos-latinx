  ...
  render() {
    ...
    { this.props.content[0] ? this.props.content[0].sections.map((plot) => { return (
          <div onKeyDown={(event) => event.keyCode == 32 ? (this.addActivePage('hidden'+plot.id), this.addActiveUnit('unit'+plot.id)) : null } tabIndex={'2'} key={plot.id} className={'treeBranch' + (this.props.currentUnit == plot.id ? ' activeTreeUnit' : '')}>  
            ...
            <div onClick={() => { this.props.content[0].sections[plot.id].content.length > 1 ? (this.addActivePage('hidden'+plot.id), this.addActiveUnit('unit'+plot.id)) : ((this.state.mobileView ? this.props.detectDimensions(false) : null), window.location.href = './#/'+plot.id+'/0') }} ref={'unit'+plot.id} className={'unitBlock'  + (this.props.currentUnit == plot.id ? ' activeUnit' : '')}>
              <h2>
