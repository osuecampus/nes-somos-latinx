          ...
          <div onKeyDown={(event) => event.keyCode == 32 ? (this.addActivePage('hidden'+plot.id), this.addActiveUnit('unit'+plot.id)) : null } tabIndex={'2'} key={plot.id} className={'treeBranch'}>  
            <div onClick={() => {this.addActivePage('hidden'+plot.id), this.addActiveUnit('unit'+plot.id)}} ref={'unit'+plot.id} className={'unitBlock'}>
              ...
              <div className={'downProgress'}>
                ...
                { localStorage.getItem('nes-progress-'+ plot.id) ?
                <CircularProgressbar
                  percentage={ (Object.keys(JSON.parse(localStorage.getItem('nes-progress-'+ plot.id))).length / this.props.content[0].sections[plot.id].content.length) * 100 }
                  styles={{
            ...
            { plot.content.map((plex) => { return (
                <div tabIndex={'2'} onKeyDown={(event) => event.keyCode == 32 ? (this.pageChoice(plot.id, plex.id), (this.state.mobileView ? this.props.detectDimensions(false) : null)) :null}  onClick={() => {this.pageChoice(plot.id, plex.id), (this.state.mobileView ? this.props.detectDimensions(false) : null)}} key={plex.id} className={'unitPage'}>
                  <a href={'#'}>{plex.title}</a>
