  ...
  render() {
    ...
    { this.props.content[0] ? this.props.content[0].sections.map((plot) => { return (
          <div onKeyDown={(event) => event.keyCode == 32 ? (this.addActivePage('hidden'+plot.id), this.addActiveUnit('unit'+plot.id)) : null } tabIndex={'2'} key={plot.id} className={'treeBranch' + (this.props.currentUnit == plot.id ? ' activeTreeUnit' : '')}>  
            {this.props.content[0].sections.length > 1 ? <div onClick={() => {this.addActivePage('hidden'+plot.id), this.addActiveUnit('unit'+plot.id)}} ref={'unit'+plot.id} className={'unitBlock'}>
              <h2>{plot.title}</h2>
            ...
            <div ref={'hidden'+plot.id} className={'hiddenPages'+ this.props.content[0].sections.length > 1 ? null : ' activePage'}>
